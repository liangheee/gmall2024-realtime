...
public class DwdTradeOrderRefundApp extends BaseSQLApp {
    public static void main(String[] args) {
        new DwdTradeOrderRefundApp().start("10017",4,Constant.TOPIC_DWD_TRADE_ORDER_REFUND);
    }
    @Override
    protected void handle(StreamTableEnvironment tableEnv) {
        tableEnv.getConfig().setIdleStateRetention(Duration.ofSeconds(5));
        // 1.1 读取 topic_db
        readTopicDb(tableEnv, Constant.TOPIC_DWD_TRADE_ORDER_REFUND);
        // 1.2 读取 字典表
        ...
        Table result = tableEnv.sqlQuery(
                "select " +
                        ...
                        "join dim_base_dic for system_time as of ri.pt as dic1 " +
                        "on ri.refund_type=dic1.dic_code " +
                        "join dim_base_dic for system_time as of ri.pt as dic2 " +
        ...
        tableEnv.executeSql(
                "create table dwd_trade_order_refund(" +
                        ...
                        ")" + SQLUtil.getUpsertKafkaSinkConnectorParams(Constant.TOPIC_DWD_TRADE_ORDER_REFUND,Constant.BROKER_SERVERS));
